...
const navItems = [
  { label: "仪表盘", href: "/console/dashboard", active: false },
  { label: "账户", href: "/console/accounts", active: true },
    ...
    <div className="min-h-screen bg-white text-[#14171a]">
      <nav className="flex h-[60px] items-center justify-between bg-[#000344] px-4 sm:px-6 lg:px-12">
        <div className="flex min-w-0 items-center gap-8 lg:gap-[60px]">
          <Link href="/console/accounts" className="shrink-0">
            <Image
          ...
          <div className="hidden items-center gap-[30px] md:flex">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="whitespace-nowrap border-b-[3px] pb-[6px] pt-[6px] text-base leading-none transition-colors hover:text-white"
                style={{
